...
export default function FoodPage() {
  ...
  const [dishes, setFood] = useState<DishEntry[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const parsedFood = (dishRows ?? []) as DishEntry[];

      const restaurantIds = Array.from(new Set(parsedFood.map((row) => row.restaurant_id).filter((id): id is string => Boolean(id))));
      ...
      if (!session?.access_token || parsedFood.length === 0) {
        setPhotoByDishEntryId({});
        return;
      }

      const ids = parsedFood.map((row) => row.id).join(',');
  ...
  return (
    <div className="space-y-2.5 pb-5">
      <section className="card-surface space-y-1.5">
        ...
        <p className="text-sm text-app-muted">Your food history with identity filters.</p>
      </section>
      ...
      <section className="space-y-1.5">
        ...
        {filteredRows.length === 0 ? (
          <p className="empty-surface">No food items yet.</p>
        ) : viewMode === 'list' ? (
          <div className="divide-y divide-app-border rounded-2xl border border-app-border bg-app-card">
            {filteredRows.map((dish) => (
              <Link key={dish.id} href={dish.dish_key ? `/food/${dish.dish_key}` : `/uploads/${dish.source_upload_id}`} className="block px-2.5 py-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {filteredRows.map((dish) => (
              <Link
                key={dish.id}
                href={dish.dish_key ? `/food/${dish.dish_key}` : `/uploads/${dish.source_upload_id}`}
